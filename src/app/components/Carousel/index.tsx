...
export default function Carousel() {
    ...
    return <section className="w-full h-[420px] flex mt-12 overflow-x-hidden max-w-screen max-h-screen pointer-events-none">
        <motion.div className="absolute left-0 select-none overflow-x-hidden flex gap-2" ref={ref} style={{ x: xTranslation, }}>
            {[...images, ...images].map((item, index) => (
                <div className="relative overflow-hidden h-[20rem] min-w-[25rem] md:h-[26rem] md:min-w-[36rem] flex justify-center items-center" key={index}>
